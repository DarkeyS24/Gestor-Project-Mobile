...
public class Cadastro extends Container {
	...
	public void initUI() {
		...
		Label lbl = new Label("Cadastro de clientes");
		...
		try {
			...
			cont.add(lbCpf, LEFT + 75, AFTER + 20, PREFERRED, PREFERRED);
			cont.add(lbCnpj, SAME, SAME, SAME + 35, SAME);
		...
		Button inserirButton = new Button("Cadastrar", Button.BORDER_OUTLINED);
		...
		inserirButton.addPressListener(new PressListener() {
			...
			public void controlPressed(ControlEvent e) {
				...
				Cliente cliente = verificarCampos();
				Boolean vf = estaCadastrado(cliente);
				if(cliente != null && vf) {
					showMessage(dao.inserirCliente(cliente));
				}
				limparCampos();
			}
		});
	...
	private Boolean estaCadastrado(Cliente cliente) {
		String msg = dao.estaCadastrado(cliente);
		if(msg == null) {
			return true;
		}else {
			showMessage(msg);
	...
	private Cliente verificarCampos() {
		String msg = ""; 
		Cliente c = null;
		...
		if(tipoCb.getSelectedIndex() == 0) {
			if(cpf.getTextWithoutMask().isEmpty() || !service.verificarCPF(cpf.getText()) || tryParseCpf()) {
				msg += "CPF Inválido\n";
			}
		}else {
			if(cnpj.getTextWithoutMask().isEmpty() || !service.verificarCNPJ(cnpj.getText()) || tryParseCnpj()) {
				msg += "CNPJ Inválido\n";
			}
		}
		...
		if(!msg.isEmpty()) {
			...
			c.setTipo(TipoCliente.valueOf(tipoCb.getSelectedItem().toString().toUpperCase()));
			c.setCpfCnpj((tipoCb.getSelectedIndex() == 0)? cpf.getText():cnpj.getText());
			c.setTelefone(telefone.getText());
			c.setEmail(email.getText().isEmpty()?null:email.getText());
		}
		
		return c;
	}
	
	private boolean tryParseCpf() {
		try {
			@SuppressWarnings("unused")
			Integer number = Integer.parseInt(cpf.getTextWithoutMask());
			return true;
		} catch (Exception e) {
			return false;
		}
	...
	@SuppressWarnings("unused")
	private boolean tryParseCnpj() {
		try {
			@SuppressWarnings("unused")
			Integer number = Integer.parseInt(cnpj.getTextWithoutMask());
			return true;
		} catch (Exception e) {
			return false;
		}
